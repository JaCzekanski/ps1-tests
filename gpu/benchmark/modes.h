...
void drawPolygons(int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if constexpr (isTextured) {
            ...
            setTPage(p, 2, 0, texture.prect->x, texture.prect->y);
...
void drawRects(int n) {
    ...
    ptr += sizeof(DR_TPAGE);
    setDrawTPage(e, 0, dithering, getTPage(2, (semiTransparency > 0) ? semiTransparency - 1 : 0, texture.prect->x, texture.prect->y));
    addToDrawList(e);
}
